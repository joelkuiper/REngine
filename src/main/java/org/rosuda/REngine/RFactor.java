...
public class RFactor {
	...
	public RFactor(int[] i, String[] v, boolean copy, int index_base) {
		...
		if (copy) {
			ids = new int[i.length];
			System.arraycopy(i, 0, ids, 0, i.length);
			levels = new String[v.length];
			System.arraycopy(v, 0, levels, 0, v.length);
		} else {
			ids = i;
			levels = v;
		}
	...
	public int[] asIntegers(int desired_index_base) {
		...
		int j = 0;
		while (j < ids.length) {
			ix[j] = ids[j] - index_base + desired_index_base;
			j++;
		}
		return ix;
	}
