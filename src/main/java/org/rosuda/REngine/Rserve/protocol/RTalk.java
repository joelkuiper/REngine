...
public class RTalk extends Observable {
	...
	public static int getLen(byte[] buf, int o) {
		...
		return ((buf[o] & 64) > 0) ? // "long" format; still - we support 32-bit only
	...
	public static long getLong(byte[] buf, int o) {
		long low = (getInt(buf, o)) & 0xffffffffL;
		long hi = (getInt(buf, o + 4)) & 0xffffffffL;
		hi <<= 32;
		hi |= low;
		return hi;
	}
