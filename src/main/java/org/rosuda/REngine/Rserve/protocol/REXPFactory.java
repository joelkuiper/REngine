...
public class REXPFactory {
	...
	public REXPFactory(REXP r) throws REXPMismatchException {
		...
		if (r instanceof REXPNull) {
			...
			if (r instanceof REXPList) {
				...
				if (r instanceof REXPGenericVector) {
					type = XT_VECTOR; // FIXME: may have to adjust names attr
				} else
					if (r instanceof REXPS4) {
						type = XT_S4;
					} else
						if (r instanceof REXPInteger) { // this includes factor - FIXME: do we need speacial handling?
							type = XT_ARRAY_INT;
						} else
							if (r instanceof REXPDouble) {
								type = XT_ARRAY_DOUBLE;
							} else
								if (r instanceof REXPString) {
									type = XT_ARRAY_STR;
								} else
									if (r instanceof REXPSymbol) {
										type = XT_SYMNAME;
									} else
										if (r instanceof REXPRaw) {
											type = XT_RAW;
										} else
											if (r instanceof REXPLogical) {
												type = XT_ARRAY_BOOL;
											} else {
	...
	public int parseREXP(byte[] buf, int o) throws REXPMismatchException {
		...
		if (xt == XT_LIST_NOTAG || xt == XT_LIST_TAG || xt == XT_LANG_NOTAG || xt == XT_LANG_TAG) {
			...
			cont = (xt == XT_LANG_NOTAG || xt == XT_LANG_TAG) ? new REXPLanguage(l, getAttr()) : new REXPList(l,
					getAttr());
			if (o != eox) {
		...
		if (xt == XT_LIST || xt == XT_LANG) { // old-style lists, for comaptibility with older Rserve versions - rather inefficient since we have to convert the recusively stored
												// structures into a flat structure
			...
			if (isRoot) {
				cont = (xt == XT_LIST) ? new REXPList(rootList, getAttr()) : new REXPLanguage(rootList, getAttr());
				rootList = null;
		...
		if (xt == XT_VECTOR || xt == XT_VECTOR_EXP) {
			...
			if (getAttr() != null && getAttr().asList().at("names") != null) {
				...
				if (nam.isString()) {
					...
					if (nam.isVector()) { // names could be a vector if supplied by old Rserve
						RList l = nam.asList();
						Object oa[] = l.toArray();
				...
				cont = (xt == XT_VECTOR_EXP) ? new REXPExpressionVector(l, getAttr()) : new REXPGenericVector(l,
						getAttr());
			} else {
				cont = (xt == XT_VECTOR_EXP) ? new REXPExpressionVector(new RList(v), getAttr())
		...
		if (xt == XT_STR || xt == XT_SYMNAME) {
			...
			try {
				if (xt == XT_STR) {
					cont = new REXPString(new String[] { new String(buf, o, i - o, RConnection.transferCharset) },
							getAttr());
				} else {
	...
	public int getBinaryLength() throws REXPMismatchException {
		...
		case XT_SYMNAME:
			...
			break;
		case XT_ARRAY_INT:
			l += cont.asIntegers().length * 4;
			break;
		case XT_ARRAY_DOUBLE:
			l += cont.asDoubles().length * 8;
			break;
		case XT_ARRAY_CPLX:
			l += cont.asDoubles().length * 8;
			break;
		case XT_ARRAY_BOOL:
			l += cont.asBytes().length + 4;
			if ((l & 3) > 0) {
				l = l - (l & 3) + 4;
			}
			break;
	...
	public int getBinaryRepresentation(byte[] buf, int off) throws REXPMismatchException {
		...
		case XT_NULL:
			break;
		case XT_INT:
			RTalk.setInt(cont.asInteger(), buf, off);
			break;
		case XT_DOUBLE:
			RTalk.setLong(Double.doubleToRawLongBits(cont.asDouble()), buf, off);
			break;
		case XT_ARRAY_INT: {
