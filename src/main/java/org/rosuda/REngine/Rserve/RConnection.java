...
public class RConnection extends REngine {
	...
	RConnection(String host, int port, RSession session) throws RserveException {
		...
		if (session == null) {
			...
			try {
				if (n != 32) { throw new RserveException(this, "Handshake failed: expected 32 bytes header, got " + n); }
				String ids = new String(IDs);
				if (ids.substring(0, 4).compareTo("Rsrv") != 0) { throw new RserveException(this,
				...
				if (rsrvVersion > 103) { throw new RserveException(this,
						"Handshake failed: The server uses more recent protocol than this client."); }
				if (ids.substring(8, 12).compareTo("QAP1") != 0) { throw new RserveException(this,
						"Handshake failed: unupported transfer protocol (" + ids.substring(8, 12)
								+ "), I talk only QAP1."); }
	...
	public RSession voidEvalDetach(String cmd) throws RserveException {
		...
		if (rp == null || !rp.isOk()) { throw new RserveException(this, "detached void eval failed", rp); }
	...
	REXP parseEvalResponse(RPacket rp) throws RserveException {
		...
		if (rsrvVersion > 100) { /* since 0101 eval responds correctly by using DT_SEXP type/len header which is 4 bytes long */
			...
			if (pc[0] != RTalk.DT_SEXP && pc[0] != (RTalk.DT_SEXP | RTalk.DT_LARGE)) { throw new RserveException(this,
					"Error while processing eval output: SEXP (type " + RTalk.DT_SEXP
							+ ") expected but found result type " + pc[0] + "."); }
			if (pc[0] == (RTalk.DT_SEXP | RTalk.DT_LARGE)) {
				rxo = 8; // large data need skip of 8 bytes
				/*
				 * warning: we are not checking or using the length - we assume that only the one SEXP is returned. This is true for the current CMD_eval implementation, but may
	...
	public RSession detach() throws RserveException {
		...
		if (rp == null || !rp.isOk()) { throw new RserveException(this, "Cannot detach", rp); }
		RSession s = new RSession(this, rp);
		close();
		return s;
	}
	...
	public REXP parseAndEval(String text, REXP where, boolean resolve) throws REngineException {
		if (where != null) { throw new REngineException(this,
				"Rserve doesn't support environments other than .GlobalEnv"); }
		try {
			return eval(text);
