...
public class RSession implements java.io.Serializable {
	...
	RSession(RConnection c, RPacket p) throws RserveException {
		host = c.host;
		rsrvVersion = c.rsrvVersion;
		byte[] ct = p.getCont();
		if (ct == null || ct.length != 32 + 3 * 4) { throw new RserveException(c,
				"Invalid response to session detach request."); }
		port = RTalk.getInt(ct, 4);
		key = new byte[32];
		System.arraycopy(ct, 12, key, 0, 32);
	}
