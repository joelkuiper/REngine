...
public class REXPFactor extends REXPInteger {
	...
	public REXPFactor(int[] ids, String[] levels) {
		...
		factor = new RFactor(payload, this.levels, false, 1);
		attr = new REXPList(new RList(new REXP[] { new REXPString(this.levels), new REXPString("factor") },
	...
	public REXPFactor(RFactor factor) {
		...
		attr = new REXPList(new RList(new REXP[] { new REXPString(levels), new REXPString("factor") }, new String[] {
				"levels", "class" }));
	}
