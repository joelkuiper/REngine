...
public class REXPGenericVector extends REXPVector {
	...
	public REXPGenericVector(RList list) {
		...
		if (payload.isNamed()) {
			attr = new REXPList(new RList(new REXP[] { new REXPString(payload.keys()) }, new String[] { "names" }));
		}
	}
	...
	/*
	 * generic vectors are converted either to a <code>Map</code> (if it is a named vector and there are no duplicate names) or a <code>Vector</code>. The contained elements are
	 * converted using <code>asNativeJavaObject</code> recursively.
	 */
